...
// 反向死锁防护: 记录"发起方id=>目标id"正在等待中的互通对。目标若反向 ask/talk 发起方 → 拒绝(见 askPersona/peerTalk)。
...
async function askPersona(targetRef, question, opts = {}) {
  ...
  const from = opts.fromName ? registry.resolveRef(opts.fromName) : null;
  // 反向死锁防护: 目标正在等发起方回复时, 你(被叫方)又反向 ask 它 → 拒绝, 提示直接文本回复。
  if (from && pendingWaits.has(target.id + '=>' + from.id)) {
    return { ok: false, error: `「${target.name}」正在等你回复 — 请直接文本回复, 不要反向调 ask_persona/talk_peer(会双向互等死锁)。` };
  }
  ...
  const waitKey = from ? (from.id + '=>' + target.id) : null;
  if (waitKey) pendingWaits.add(waitKey);
  try {
    const answer = await s.butler.askOnce(wrapped);
    return { ok: true, answer, from: target.name };
  } catch (e) { return { ok: false, error: String(e && e.message) }; }
  finally { if (waitKey) pendingWaits.delete(waitKey); }
}

// 叶子↔叶子直连(需用户授权): 直接对话(非管家中转)+ 抄送管家(方案B: 记录到管家记忆, 管家随时可查, 不打断)。
async function peerTalk(fromName, targetRef, message, opts = {}) {
  ...
  // 反向死锁防护: 目标正在等发起方回复时, 你又反向 talk_peer 它 → 拒绝, 提示直接文本回复。
  if (pendingWaits.has(target.id + '=>' + from.id)) {
    return { ok: false, error: `「${target.name}」正在等你回复 — 请直接文本回复, 不要反向调 talk_peer/ask_persona(会双向互等死锁)。` };
  }
  const depth = (opts.depth || 0) + 1;
  if (depth > 3) return { ok: false, error: '防环: depth > 3, 当前调用链太深' };
  let s = sessions.get(sidOf(target.homeDir));
  ...
  if (!s) { s = openPersona(target.homeDir); sendUI('persona-opened', { meta: metaOf(s) }); }
  console.error(`[dbg peerTalk] ${target.name} session ready (butler._q=${!!s.butler._q}) → askOnce...`);
  const wrapped = `【${from.name} 通过直连(talk_peer)对你说 · depth=${depth}】\n\n${message}\n\n(你的答复会返回给 ${from.name}; 本次对话已抄送管家, 用户与管家可见)`;
  const waitKey = from.id + '=>' + target.id;
  pendingWaits.add(waitKey);
  try {
    const answer = await s.butler.askOnce(wrapped);
    console.error(`[dbg peerTalk] ${target.name} askOnce RETURNED len=${(answer || '').length}`);
    ccButler(from.name, target.name, message, answer);
    return { ok: true, answer, from: target.name };
  } catch (e) { console.error(`[dbg peerTalk] ${target.name} askOnce ERROR: ${e && e.message}`); return { ok: false, error: String(e && e.message) }; }
  finally { pendingWaits.delete(waitKey); }
}

// 抄送管家(方案B): 叶子间这轮对话追加到管家记忆目录的 peer_cc_log.md → 管家随时可查(掌握全局), 不打断。
function ccButler(fromName, toName, message, answer) {
  try {
...
function onboardingSteps(name, extra) {
  ...
  steps.push(`还有一条【协作规矩(星型)】：这里有一位【管家】在中心。你有事——问别的专家、要协调、找资源——先找管家(ask_persona 只能找管家)。你和其他非管家人格之间【默认不直接通话】：既避免乱套，也让用户只跟管家聊就能掌握全局。确实要和某个人格直接对话时，得【用户授权 + 管家开通】，之后才能直连(且对话会抄送管家)。记住了回我一声。`);
